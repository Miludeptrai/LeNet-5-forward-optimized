...
{
    int t = blockIdx.x * blockDim.x + threadIdx.x; //
    int height_unroll = height_out * width_out; //
    ...
    __syncthreads();
    if (t < height_out * width_out * channel_out) {
        int r = t / channel_out;
        int c = t % channel_out ; 
        //if (r < m && c < k) {
            float sum = 0 ;
...
__host__ void Kernel_none_optimize::conv_forward_gpu_full( int n_samples,  int channel_in,  int height_in, int width_in,
    ...
    dim3 num_threads_per_block_1(1024);
    dim3 num_blocks_in_grid_1((height_out * width_out  * channel_in-1)/1024 + 1 );

    dim3 blockSize(32, 32);
    dim3 gridSize(( channel_out-1)/blockSize.y + 1,(height_out * width_out-1)/blockSize.x + 1,1);
